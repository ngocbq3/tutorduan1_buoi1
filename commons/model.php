...
function searchProductInCatalogue($name, $cate_id)
{
    try {
        $sql = "SELECT p.* FROM products p JOIN catalogues c ON p.catalogue_id=c.id WHERE p.name LIKE '%$name%'";
        if ($cate_id != '') {
...
function filterOfPrice($min, $max)
{
    ...
    if ($min == null) {
        $sql .= " price_regular < $max";
    } else if ($max == null) {
        $sql .= " price_regular > $min";
    } else {
        $sql .= " price_regular >= $min AND price_regular <= $max";
    }
    $stmt = $GLOBALS['conn']->prepare($sql);
    $stmt->execute();
    $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
